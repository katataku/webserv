...
aliasを解決すると、`/somepath/hoge`となる。

1. `/somepath`が存在しない場合

<!-- - TODO(iyamada) どうするんだろう -->

2. `/somepath/hoge`が存在しない場合

- `/somepath/hoge`というファイルを新規作成し、メッセージボディを書き込む
- ステータスコードは`201`を返す

3. `/somepath/hoge`がディレクトリの場合

- 適当な名前でファイルを新規作成し、メッセージボディを書き込む
- ステータスコードは`201`を返す

4. `/somepath/hoge`がファイルの場合

- ファイルの末尾にメッセージボディを追加する
- ステータスコードは`200`を返す

失敗したらステータスコードは`500`を返す
